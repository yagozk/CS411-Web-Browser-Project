...
const BrowserToolbar = ({onBookmarkClick, setBookmarkName}) => {
    ...
    const BookmarkPopup = () => {
        if (isBookmarkPopupShowing && folders.length === 1 && folders[0].bookmarks.length === 0) return (
            <div className={"bookmark-dropdown"} style={{padding: 5}}>
                No bookmarks yet!
            </div>)
        if (isBookmarkPopupShowing && folders.length > 0) return (<div className={"bookmark-dropdown"}>
            <TreeView
                defaultCollapseIcon={<ExpandMoreIcon/>}
                defaultExpandIcon={<ChevronRightIcon/>}
            >
                {folders.map((folder, i) => (folder.name === "Parent" ? (<>
                    {folder.bookmarks.map((bookmark, j) => (
                        <TreeItem nodeId={bookmark.name} label={bookmark.name} onClick={() => {
    ...
    return (<nav className="navbar navbar-expand-lg navbar-light bg-dark text-white">
        <div className="container">
            <div className="navbar-brand text-white logoText" style={{marginLeft: "-100px", marginRight: '100px'}} href="/">
                <img
                    alt=""
            ...
            <div className="collapse navbar-collapse" id="basic-navbar-nav">
                <div className="input-group me-1">
                    <div className="input-group-prepend">
                      <span className="input-group-text" id="basic-addon1" style={{borderRadius: "5px 0px 0px 5px"}}>
                        Address:
                      </span>
                    </div>
                    <LinkInputField value={currentAddress} onChange={(e) => setCurrentAddress(e.target.value)}
